...
# if torch.allclose(triton_output, torch_output, atol=7e-2, rtol=0):
...
for i in range(10):
    configs.append(triton.Config(
        ...
        num_stages=random.randrange(2, 8, 1), num_warps=random.randrange(2, 8, 2)
    ))

def benchmark(M, N, K, provider, activation):
    ...
    perf = lambda x: round((2 * M * N * K * 1e-12 / (x * 1e-3)), 3)
    ...
    if provider == 'cublas + {}'.format(activation):
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: activation_map[activation](torch.matmul(a, b)), quantiles=quantiles)
    
    if provider == 'triton + {}'.format(activation):   
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: matmul(a, b, activation=activation), quantiles=quantiles)
